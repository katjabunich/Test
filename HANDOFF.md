...
Новая страница `/stats` — «зеркало продуктивности» в формате дневника, не дешборда. Эмоциональный retention-механизм. **Никаких корпоративных бар-графиков** — у DoIt тёплая paper-эстетика, нужен соответствующий визуал.

#### Размещение
- **5-я вкладка в BottomNav** между Привычки и Настройки
- Иконка: `BarChart` в стиле Icons.tsx (3 вертикальных полоски разной высоты, lucide-style 1.8 stroke)
- Маршрут `/stats`

#### Структура страницы (сверху вниз)

**1. Hero — «число периода»**
```
              23                    ← Lora 700, 96px, var(--ink)
        завершено за неделю          ← Manrope 500, 13px, var(--ink-60)
       ↑ на 5 больше, чем неделей раньше   ← 12px, var(--ink-60), стрелка цветом
```
- При смене таба цифра roll-up'ится от 0 → значение за 600ms (cubic-bezier(0.22, 1, 0.36, 1))
- Сравнительная строка опциональна — показываем только если есть прошлый период с данными
- Стрелка ↑ зелёная (mint-deep) если рост, ↓ серая если падение, `~` если как обычно (±1)

**2. Tabs — текстовые с подплывающим underline**
```
   СЕГОДНЯ      НЕДЕЛЯ      МЕСЯЦ
                ━━━━━━
```
- Не «pill» табы — текст заглавными letter-spacing 0.04em, Manrope 600 13px
- Активный таб: var(--ink), неактивный: var(--ink-40)
- Тонкий мятный underline (2px var(--mint-deep)) скользит между табами на 280ms transition
- Свайп между табами должен работать как альтернативная навигация (touch sensors)

**3. Раскладка по сферам — точки-бусины**
Каждая задача = одна цветная точка (8x8px, radius 2px, цвет сферы):
```
Работа    ●●●●●●●●                              8
Дом       ●●●●●●●●●●●●                          12
Личное    ●●●                                    3
```
- Имя сферы — Manrope 600 13px var(--ink-80)
- Точки flex-wrap при переполнении строки
- Цифра справа — Manrope 600 14px tabular-nums
- Если в сфере >50 задач — fallback на тонкий бар (height 8px, цвет сферы, ширина пропорциональна max в категории)
- Stagger-in анимация при появлении: 30ms между точками, max 0.9s total
- **Тап на строку сферы** → фильтрует список ниже только этой сферой (визуальный indicator активного фильтра + кнопка «Сбросить»)

**4. Mini-calendar (только для Неделя/Месяц)**

**Для «Неделя»** — 7 столбиков, каждый день вертикальная стопка точек по сферам:
```
...
сегодня помечен мятным underline
```

**Для «Месяц»** — сетка 7×5 (или 6 рядов в зависимости от месяца), цветовая «теплота» в paper-warm → butter → peach gradient по числу выполненного. Это не GitHub-grey-scale, это тёплая акварельная gradient.
- Размер ячейки: ~44x44px (touch-friendly минимум)
- Внутри ячейки: число дня (Manrope 500 11px, var(--ink-40))
- Сегодня: мятная мини-точка в углу
- Пустой день: не нулевая ячейка, paper-deep как неактивная

**Тап на день** → список ниже scroll-ится к этому дню + soft mint border highlight на 600ms

**5. Список «Что сделано» — карточки-дни**

Группировка по дню. Каждый день — soft card на paper-warm:
```
┌─────────────────────────────────────┐
│ Понедельник, 6 мая          5      │  ← Lora 600 18px
│                                     │
│ ✓ Купить молоко                    │  ← Manrope 500 15px
│   🏡 Дом · 09:24                    │  ← Manrope 500 12px var(--ink-60)
│                                     │
│ ✓ Позвонить маме                   │
│   🌿 Личное · 14:12                 │
└─────────────────────────────────────┘
...
- Day-header — Lora 600 18px (премиум-серифный почерк) + цифра-счётчик справа Manrope 600
- Чек ✓ в цвете сферы (var(--mint-deep) если без сферы)
- Сферный индикатор: маленький emoji + название
- Время выполнения: HH:MM в локальном TZ юзера (`completed_at` хранится в UTC, конвертим)
- **Тап на задачу** → expand вниз, показывает note (если есть) + sphere chip + due-date (если был). Spring-анимация 280ms (cubic-bezier(0.34, 1.4, 0.64, 1))
- **Пустые дни (0 завершено) НЕ пропускаем** — показываем editorial-фразу:
```
┌─────────────────────────────────────┐
│ Воскресенье, 5 мая          0      │
│                                     │
│   Тишина. Хороший день для отдыха.  │  ← Lora italic 14px var(--ink-40)
└─────────────────────────────────────┘
```
- Editorial-фразы для пустых дней — варьируй из набора:
  - «Тишина. Хороший день для отдыха.»
  - «День без задач. Тоже бывает.»
  - «Пусто. Иногда так нужно.»
  - «Без галочек. Надеюсь, прожила хорошо.»
  - Выбираем псевдо-рандомно по hash от даты (детерминированно — одну дату всегда отрисовываем одной фразой)

#### Empty states (полностью пустой период)

- **Сегодня (0)**: «Сегодня пока ничего завершено. Не страшно — иногда нужен отдых.»
- **Неделя (0)**: «На этой неделе тихо. Скоро будет о чём вспомнить.»
- **Месяц (0)**: «Месяц только начинается. Самое интересное впереди.»

Без иллюстраций (мы решили не доверять SVG-картинкам). Только текст в Lora 16px italic, центрирован, padding 40px.

#### Жесты

- Свайп влево/вправо между табами (`useSwipeable` или manual touch handlers)
- Pull-to-refresh с haptic-tick (если doable)
- Long-press на день в месяц-сетке → peek-popover (опционально v2, на v1 оставить простой tap)

#### Данные

- **`fetchCompletedTasks(startIso: string, endIso: string): Task[]`** — новая функция в `src/lib/db.ts`
  - `select * from tasks where user_id = X and completed_at between $1 and $2 and completed_at is not null order by completed_at desc limit 200`
  - Спокойно бьёт RLS, обычный user-scoped client
- Конвертация дат «сегодня/неделя/месяц» в start/end делается на КЛИЕНТЕ (знает локальный TZ), отправляется ISO строкой
- Группировка по `sphere_id` для бусин-сфер
- Группировка по дате (YYYY-MM-DD в локальном TZ) для дневного списка

#### Файлы для создания/изменения

| Файл | Что |
|---|---|
| `src/app/stats/page.tsx` | Server component, fetchSpheres + initial data fetch для текущего таба (Сегодня) |
| `src/app/stats/StatsView.tsx` | Client, табы, раскладка по сферам, мини-календарь, dayCards |
| `src/lib/db.ts` | `fetchCompletedTasks(startIso, endIso)` |
| `src/components/BottomNav.tsx` | 5-я иконка `BarChart` |
| `src/components/Icons.tsx` | Новая иконка `BarChart` (3 вертикальных бара 6, 12, 9 высоты) |
| `src/lib/i18n/dict.ts` | Новый раздел `stats.*`: title, sub, tab_today/week/month, completed_count, empty_*, day_emptyN (5 фраз) |

#### Edge cases

- 0 завершённых в периоде → editorial empty state
- Очень много (>100) → пагинация по 30 в день, кнопка «Показать ещё»
- Сферы которые удалены (sphere_id null или RLS не пускает) → пометить как «Без сферы», cерый цвет (var(--ink-40))
- Часовой пояс: completed_at в UTC, надо конвертировать в локальный для группировки по дням
- Если задача завершена в воскресенье 23:55 локально (понедельник 02:55 UTC), она должна попасть в воскресенье — конвертация на клиенте

#### Анимация принципы

- Roll-up счётчика: 600ms ease-out
- Stagger бусин: 30ms интервал
- Tab underline slide: 280ms cubic-bezier(0.22, 1, 0.36, 1)
- Day card tap expand: spring 280ms cubic-bezier(0.34, 1.4, 0.64, 1)

#### Что НЕ делаем (намеренно)

- ❌ Бары/штриховые графики — корпоративно
- ❌ Проценты («87% завершено») — клинично
- ❌ Линейные графики per-day — мини-календарь это покрывает
- ❌ Бейджи/ачивки/уровни — дешевит
- ❌ Sharing — это интимное приложение
- ❌ Сравнение с другими юзерами — плохой паттерн
- ❌ «Letter to yourself» editorial-блок (пользователь решил не делать)

#### Эстимейт: 1.5-2 часа фокусированной работы.
