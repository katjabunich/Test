...
Новая страница `/stats` — «зеркало продуктивности». Эмоциональный retention-механизм + premium-сигнал.

**Где живёт:**
- 5-я вкладка в BottomNav (иконка BarChart, между Привычки и Настройки)
- Маршрут `/stats`
...
Большая цифра: 23
Подпись: «задач выполнено»

Раскладка по сферам — горизонтальные цветные бары:
🟠 Работа     ████████░░ 8
🟢 Дом        ██████████ 12
🟣 Личное     ███░░░░░░░ 3

Раздел «Что сделано» — список завершённых задач, группированный по дням:
— Сегодня
   ✓ Купить молоко (Дом · 09:24)
   ✓ Позвонить маме (Личное · 14:12)
— Вчера
   ✓ ...
```

**Данные:**
- Используем существующее поле `tasks.completed_at TIMESTAMPTZ`
- Никаких миграций
- Новая функция в `src/lib/db.ts`: `fetchCompletedTasks(startIso, endIso)` — `where user_id = X and completed_at between ... order by completed_at desc limit 100`

**Часовые пояса:**
- Конвертация дат «сегодня/неделя/месяц» в start/end делается на клиенте (знает локальный TZ), посылается ISO в server
- Server просто фильтрует по UTC

**Файлы для изменений:**
- `src/app/stats/page.tsx` — server, fetchSpheres + fetchCompletedTasks для выбранного периода
- `src/app/stats/StatsView.tsx` — client, табы (state-driven, не URL-driven для скорости), бары, список
- `src/lib/db.ts` — `fetchCompletedTasks`
- `src/components/BottomNav.tsx` — 5-я иконка
- `src/components/Icons.tsx` — `BarChart` icon (обычный lucide-style: 3 вертикальных бара разной высоты)
- `src/lib/i18n/dict.ts` — ключи `stats.title`, `stats.tab_today`, `stats.tab_week`, `stats.tab_month`, `stats.completed_count`, `stats.empty`, `stats.section_done`

**Edge cases:**
- 0 завершённых → empty state: «За этот период ничего не завершено. Иногда нужен отдых.»
- Очень много (>50) → ограничить рендер списка, показать «+N ещё»
- Сферы которые удалены (RLS не пускает к ним) → пометить как «Без сферы» с серым цветом
